...
SELECT /*+ &&sq_fact_hints. */
 ...
 WHERE eadam_seq_id = &&eadam_seq_id.
   ...
   AND stat_name IN (''NUM_CPUS'', ''LOAD'', ''IDLE_TIME'', ''BUSY_TIME'', ''USER_TIME'', ''NICE_TIME'', ''SYS_TIME'', ''OS_CPU_WAIT_TIME'', ''RSRC_MGR_CPU_WAIT_TIME'', ''IOWAIT_TIME'', ''VM_IN_BYTES'', ''VM_OUT_BYTES'')
   AND snap_id BETWEEN &&minimum_snap_id. AND &&maximum_snap_id.
...
BEGIN
  FOR i IN 1 .. 15
  LOOP
    SELECT COUNT(*) INTO l_count FROM dba_hist_database_instanc_s WHERE eadam_seq_id = &&eadam_seq_id. AND instance_number = i AND ROWNUM = 1;
    IF l_count = 0 THEN
...
SELECT /*+ &&sq_fact_hints. */
 ...
 WHERE eadam_seq_id = &&eadam_seq_id.
   AND dbid = &&eadam_dbid.
   AND stat_name IN (''NUM_CPUS'', UPPER(''@stat_name@''))
   AND snap_id BETWEEN &&minimum_snap_id. AND &&maximum_snap_id.
 GROUP BY
       snap_id,
       dbid,
